...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/10/25 15:26:58 by mmahias           #+#    #+#             */
/*   Updated: 2021/10/27 15:01:21 by mmahias          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	data->acr_fd1 = ft_access_read(av[1]);
	ft_found_path(data);
	ft_dup_path(data);
	ft_split_cmds(data, av);
	ft_pathname_cmd(data);
	exc_or_abs(data);
	if (data->abs1 == 0 && data->exc1 == 0 && data->fd1 > -1)
		data->acces_cmd1 = ft_found_access(data->pathname1, \
		data->cmds1[0], data);
	if (data->abs2 == 0 && data->exc2 == 0)
		data->acces_cmd2 = ft_found_access(data->pathname2, \
		data->cmds2[0], data);
}
...
void	ft_pipex(t_data *data, char **av)
{
	...
	if (child1 < 0)
		...
		child1_process(data->fd1, data, pipefd);
	child2 = fork();
	if (child2 < 0)
		exit(1);
	else if (child2 == 0)
		child2_process(data->fd2, data, pipefd);
	close(pipefd[0]);
	close(pipefd[1]);
